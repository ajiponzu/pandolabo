 ...
 * These structures provide high-level abstractions for Vulkan resource
 * descriptions.
 */
...
struct ImageSubInfo {
  gpu_ui::GraphicalSize<uint32_t>
      graphical_size{};        ///< Width and height dimensions of the image
...
struct DescriptorInfo {
  vk::ShaderStageFlags
      stage_flags{};      ///< Which shader stages can access this resource
  uint32_t binding = 0u;  ///< Binding index in the descriptor set layout
  vk::DescriptorType
      type{};          ///< Type of descriptor (uniform buffer, sampler, etc.)
...
struct PushConstantRange {
  vk::ShaderStageFlags
      stage_flags{};  ///< Which shader stages can access these push constants
...
/// @details This struct is not only for image view, but also for image barrier,
/// etc. It's useful to manage image's miplevel and array layers.
struct ImageViewInfo {
  uint32_t base_mip_level =
      0u;  ///< First mipmap level accessible through this view
  uint32_t mip_levels = 0u;  ///< Number of mipmap levels accessible
                             ///< (VK_REMAINING_MIP_LEVELS for all)
  uint32_t base_array_layer =
      0u;  ///< First array layer accessible through this view
  uint32_t array_layers = 0u;  ///< Number of array layers accessible
                               ///< (VK_REMAINING_ARRAY_LAYERS for all)
  ImageAspect
      aspect;  ///< Which aspects of the image to access (color, depth, stencil)
...
struct SamplerInfo {
  ...
  SamplerAddressMode
      address_mode_u;  ///< Addressing mode for U coordinate (outside [0,1])
  SamplerAddressMode
      address_mode_v;  ///< Addressing mode for V coordinate (outside [0,1])
  SamplerAddressMode
      address_mode_w;  ///< Addressing mode for W coordinate (outside [0,1])
  ...
  float_t max_anisotropy =
      0.0f;  ///< Maximum anisotropy ratio (1.0-16.0 typically)
  bool compare_enable =
      false;  ///< Whether to enable comparison mode (for shadow mapping)
  SamplerCompareOp
      compare_op;          ///< Comparison operation when compare_enable is true
  float_t min_lod = 0.0f;  ///< Minimum level-of-detail parameter
  float_t max_lod = 0.0f;  ///< Maximum level-of-detail parameter
  SamplerBorderColor
      border_color;  ///< Border color when using clamp-to-border addressing
  bool unnormalized_coordinates =
      false;  ///< Whether to use unnormalized coordinates [0,size] vs [0,1]
  ...
  SamplerInfo& setAddressModes(SamplerAddressMode u,
  ...
  SamplerInfo& setCompare(bool enable,
                          SamplerCompareOp op = SamplerCompareOp::Never) {
