/*
 * buffer_helpers.hpp - Buffer creation helper functions for Pandolabo core module
 *
 * This header contains utility functions for creating various types of GPU buffers
 * including staging, storage, uniform, vertex, and index buffers.
 */
...
/// Creates a buffer optimized for transferring data from CPU memory to GPU memory.
/// @param ptr_context GPU context for device access
/// @param size Buffer size in bytes
/// @return Buffer object configured for CPU-to-GPU transfer
gpu::Buffer createStagingBufferToGPU(const std::unique_ptr<gpu::Context>& ptr_context, const size_t size);

/// @brief Create staging buffer for CPU to GPU data transfer (unique_ptr version)
/// Provides explicit ownership management for cases where heap allocation is needed.
/// @param ptr_context GPU context for device access
/// @param size Buffer size in bytes
/// @return Unique pointer to buffer object configured for CPU-to-GPU transfer
std::unique_ptr<gpu::Buffer> createUniqueStagingBufferToGPU(const std::unique_ptr<gpu::Context>& ptr_context,
                                                            const size_t size);

/// @brief Create staging buffer for GPU to CPU data transfer
/// Creates a buffer optimized for transferring data from GPU memory to CPU memory.
/// @param ptr_context GPU context for device access
/// @param size Buffer size in bytes
/// @return Buffer object configured for GPU-to-CPU transfer
gpu::Buffer createStagingBufferFromGPU(const std::unique_ptr<gpu::Context>& ptr_context, const size_t size);

/// @brief Create staging buffer for GPU to CPU data transfer (unique_ptr version)
/// Provides explicit ownership management for cases where heap allocation is needed.
/// @param ptr_context GPU context for device access
/// @param size Buffer size in bytes
/// @return Unique pointer to buffer object configured for GPU-to-CPU transfer
std::unique_ptr<gpu::Buffer> createUniqueStagingBufferFromGPU(const std::unique_ptr<gpu::Context>& ptr_context,
                                                              const size_t size);
...
gpu::Buffer createStorageBuffer(const std::unique_ptr<gpu::Context>& ptr_context,
                                TransferType transfer_type,
                                const size_t size);

/// @brief Create storage buffer for compute shader operations (unique_ptr version)
/// Provides explicit ownership management for cases where heap allocation is needed.
/// @param ptr_context GPU context for device access
/// @param transfer_type Transfer capabilities needed for this buffer
/// @param size Buffer size in bytes
/// @return Unique pointer to buffer object configured for storage operations
std::unique_ptr<gpu::Buffer> createUniqueStorageBuffer(const std::unique_ptr<gpu::Context>& ptr_context,
                                                       TransferType transfer_type,
                                                       const size_t size);
...
gpu::Buffer createUniformBuffer(const std::unique_ptr<gpu::Context>& ptr_context, const size_t size);
...
std::unique_ptr<gpu::Buffer> createUniqueUniformBuffer(const std::unique_ptr<gpu::Context>& ptr_context,
                                                       const size_t size);
...
gpu::Buffer createVertexBuffer(const std::unique_ptr<gpu::Context>& ptr_context, const size_t size);
...
std::unique_ptr<gpu::Buffer> createUniqueVertexBuffer(const std::unique_ptr<gpu::Context>& ptr_context,
                                                      const size_t size);
...
gpu::Buffer createIndexBuffer(const std::unique_ptr<gpu::Context>& ptr_context, const size_t size);
...
std::unique_ptr<gpu::Buffer> createUniqueIndexBuffer(const std::unique_ptr<gpu::Context>& ptr_context,
                                                     const size_t size);
