/*
 * command_buffer.hpp - Command buffer and command management for Pandolabo core
 * module
 *
...
class CommandBeginInfo {
 private:
  vk::RenderPass
      m_renderpass{};  ///< Render pass for inheritance (secondary buffers)
  vk::Framebuffer
      m_framebuffer{};  ///< Framebuffer for inheritance (secondary buffers)
...
/// types (transfer, compute, graphics) and manages the underlying Vulkan
/// command buffer.
/// @note This is not an abstract class - it can be instantiated directly for
/// basic operations
class CommandBuffer {
 protected:
  ...
  vk::CommandBuffer
      m_commandBuffer;         ///< Underlying Vulkan command buffer handle
 ...
 public:
  ...
  /// @param begin_info Configuration for how the command buffer should be
  /// started
  ...
  /// Push constants provide a fast way to pass small amounts of data to
  /// shaders.
...
/// This class provides an interface for GPU transfer commands including
/// buffer-to-buffer copies, buffer-to-image transfers, and mipmap generation
/// operations.
class TransferCommandBuffer : public CommandBuffer {
 ...
 public:
  ...
  /// @param staging_buffer Source buffer containing data (typically
  /// CPU-accessible)
  ...
  /// Destination owner command buffer must call this function for queue family
  /// ownership transfer.
...
/// This class provides an interface for GPU compute commands, extending
/// transfer capabilities with compute shader execution functionality.
...
/// including render pass management, drawing operations, and dynamic state
/// updates.
class GraphicCommandBuffer : public ComputeCommandBuffer {
 ...
 public:
  ...
  void bindVertexBuffer(const gpu::Buffer& buffer,
...
class CommandDriver {
 private:
  ...
  vk::UniqueCommandBuffer
      m_ptrPrimaryCommandBuffer;  ///< Primary command buffer
  std::vector<vk::UniqueCommandPool>
      m_ptrSecondaryCommandPools;  ///< Secondary command pools
  std::vector<vk::UniqueCommandBuffer>
      m_secondaryCommandBuffers;  ///< Secondary command buffers

  QueueFamilyType
      m_queueFamilyType;  ///< Queue family type (graphics, compute, transfer)
 ...
 public:
  ...
  GraphicCommandBuffer getGraphic(
