...
pandora::core::gpu::SolidBinarySemaphore::SolidBinarySemaphore(const std::unique_ptr<Context>& ptr_context) {
  ...
  m_ptrFence = ptr_vk_device->createFenceUnique(vk::FenceCreateInfo(vk::FenceCreateFlagBits::eSignaled));
}
...
void pandora::core::gpu::SolidBinarySemaphore::wait(const std::unique_ptr<Context>& ptr_context) {
  const auto& ptr_vk_device = ptr_context->getPtrDevice()->getPtrLogicalDevice();

  const auto vk_result = ptr_vk_device->waitForFences(m_ptrFence.get(), VK_TRUE, std::numeric_limits<uint64_t>::max());
  if (vk_result != vk::Result::eSuccess) {
    throw std::runtime_error("Failed to wait for fence.");
  }
}

pandora::core::gpu::AcquireImageSemaphore::AcquireImageSemaphore(const std::unique_ptr<Context>& ptr_context) {
...
pandora::core::gpu::RenderSemaphore::RenderSemaphore(const std::unique_ptr<Context>& ptr_context) {
