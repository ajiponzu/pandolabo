...
pandora::core::CommandDriver::CommandDriver(
...

  {
    ...
    m_ptrCommandPool =
        ptr_device->getPtrLogicalDevice()->createCommandPoolUnique(pool_info);
  }
  ...
  m_ptrPrimaryCommandBuffer =
      std::move(ptr_device->getPtrLogicalDevice()
...
void pandora::core::CommandDriver::constructSecondary(
  ...
  for (uint32_t idx = 0u; idx < required_secondary_num; idx += 1u) {
    ...
    m_secondaryCommandBuffers.push_back(std::move(
        ptr_vk_device
...
void pandora::core::CommandDriver::resetAllCommandPools(
  ...
  ptr_vk_device->resetCommandPool(m_ptrCommandPool.get(),
